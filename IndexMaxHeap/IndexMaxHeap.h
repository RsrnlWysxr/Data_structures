...
private:            // 默认i为真实数组的索引，j为index的索引，即j为真实顺序
    T* data;        // 最大索引堆中的数据    根据i访问具体数据
    int* index;     // 最大索引堆中的索引    index[j] = i        // 也是真正的堆
    int* reverse;   // 最大索引堆中的反向索引  reverse[i] = j
    int size;       // 数据数量
    int capacity;   // 数组空间
...
public:
    ...
    explicit IndexMaxHeap(int capacity):capacity(capacity)
    {
        size = 0;
        data = new T[capacity];
